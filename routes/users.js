...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
      cb(null, classifierFolder); // Save files in classifier folder
  },
  filename: (req, file, cb) => {
      cb(null, 'peta_pdf.pdf'); // Rename file to peta_pdf.pdf
  }
});
...
router.post('/get_question', async function(req, res, next) {
  try {
    ...
    answer = answer.replace(/Confidence: \d+%/, "").replace(/Sources: Pages [\d, ]+/, "").trim();
...
// Invoke-RestMethod -Uri "http://127.0.0.1:8000/ask_question" -Method Post -ContentType "application/json" -Body '{"question": "who wrote the odyssey?"}'
